...
<table>
    ...
    <tr>
       <td>{{$departamento->nombre}}</td><td>{{$departamento->localizacion}}</td><td>{{$departamento->jefe->nombre ?? "No existe un jefe"}}</td><td><a href="{{url('departamento/' . $departamento->id)}}">show</a></td><td><a href="{{url('departamento/' . $departamento->id . '/edit')}}">edit</a></td>
       <td>
